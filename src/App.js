...
export default function App() {
  ...
  if (submitted) return <StatusPage username={submitted} onBack={() => setSubmitted('')} />;
  ...
  return (
    <>
      ...
      <div style={{position:'fixed',inset:0,zIndex:0,overflow:'hidden',pointerEvents:'none'}}>
        ...
        <div style={{position:'absolute',width:'300px',height:'300px',borderRadius:'50%',background:'radial-gradient(circle,rgba(34,197,94,0.06) 0%,transparent 70%)',top:'40%',left:'60%',animation:'drift1 18s ease-in-out infinite reverse'}} />
      ...
      <div style={{position:'relative',zIndex:1,minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'40px 24px'}}>
        ...
        <p className="hero-sub" style={{color:'#64748b',fontSize:'18px',textAlign:'center',marginBottom:'48px',maxWidth:'480px',lineHeight:'1.6'}}>
          Beautiful public status pages powered by AWS ECS, Redis, and Lambda — built for developers.
        </p>

        {/* Main card */}
        <div className="hero-card" style={{position:'relative',width:'100%',maxWidth:'420px',marginBottom:'48px'}}>
          {/* Card glow */}
          <div style={{position:'absolute',inset:'-1px',borderRadius:'17px',background:'linear-gradient(135deg,rgba(59,130,246,0.3),rgba(139,92,246,0.3),rgba(34,197,94,0.1))',zIndex:-1,filter:'blur(8px)',opacity:0.6}} />
          ...
          <div style={{background:'rgba(10,15,30,0.85)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'16px',padding:'36px',backdropFilter:'blur(24px)'}}>
            ...
            <p style={{color:'#475569',fontSize:'13px',marginBottom:'20px',lineHeight:'1.5'}}>
              Enter a username to view their live status page
            </p>
            ...
            <div style={{position:'relative',marginBottom:'12px'}}>
              ...
              <input
                ...
                onKeyDown={e => e.key === 'Enter' && username && setSubmitted(username)}
              />
            </div>

            <button
              className="view-btn"
              style={{width:'100%',padding:'14px',borderRadius:'10px',border:'none',background:'linear-gradient(135deg,#3b82f6,#6d28d9)',color:'#fff',fontSize:'15px',fontWeight:'700',cursor:'pointer',letterSpacing:'0.2px',boxShadow:'0 4px 16px rgba(59,130,246,0.3)'}}
              onClick={() => username && setSubmitted(username)}
            >
              View Status Page →
            </button>
        ...
        <div className="hero-stats" style={{display:'flex',gap:'16px',flexWrap:'wrap',justifyContent:'center'}}>
          {[
            { icon: '⚡', label: 'Real-time monitoring', sub: 'Every 60 seconds' },
            { icon: '🔔', label: 'Instant alerts', sub: 'Email via AWS SNS' },
            { icon: '☁️', label: 'Fully serverless', sub: 'ECS + Lambda + Redis' },
          ].map((item, i) => (
